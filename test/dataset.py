...
class CorrespondencesDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.config.use_ratio == 0 and self.config.use_mutual == 0:
            ...
            xs = xs[:,mask,:]
            ys = ys[mask,:]
        elif self.config.use_ratio == 1 and self.config.use_mutual == 1:
            ratio_mask = np.asarray(self.data['ratios'][str(index)]).reshape(-1) < self.config.ratio_test_th
            mutual_mask = np.asarray(self.data['mutuals'][str(index)]).reshape(-1).astype(bool)
