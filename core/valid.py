...
def valid_process(mode, model, cur_global_step, data_loader, config):
    ...
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            res_logits, res_e_hat = model(test_data, training=False)
    ...
    prior_summary = {key: float(np.mean(np.asarray(values))) for key, values in prior_info.items()}
    return [ret_val, np.mean(np.asarray(network_info['geo_losses'])), np.mean(np.asarray(network_info['cla_losses'])), \
        ...
        np.mean(np.asarray(network_info['recalls'])), np.mean(np.asarray(network_info['f_scores'])), prior_summary]
