            ...
            for seq in self.seqs:
                ...
                for tp in data_type:
                    data_path = os.path.join(
                        self.dump_dir, seq, self.desc_name, self.mode, str(tp) + '.pkl'
                    )
                    with open(data_path, 'rb') as data_file:
                        data_seq[tp] = pickle.load(data_file)
    ...
    def dump_data(self):
        ...
        for seq in self.seqs:
            ...
            if self.pair_path is not None:
                pair_name = os.path.join(
                    self.pair_path,
                    seq.rstrip('/\\') + '-te-' + str(self.pair_num) + '-pairs.pkl',
                )
            dataset_path = os.path.join(self.dataset_path, seq, self.mode)
            dump_dir = os.path.join(self.dump_dir, seq, self.desc_name, self.mode)
            print(dataset_path)
